...
def run_test_even_better_draw_circles():
    even_better_draw_circles(3, 10, .1)
    even_better_draw_circles(51, 5, .01)
    even_better_draw_circles(101, 1, .001)


def even_better_draw_circles(num_of_circles, change_in_radius, draw_delay):
    """
    :type num_of_circles: int
    :type change_in_radius: int
    :type draw_delay: float

    :param num_of_circles:
    :param chang_in_radius:
    :param draw_delay:
    :return:
    """
    ...
    for k in range(num_of_circles):
        circle = rg.Circle(center, change_in_radius * k)
        circle.attach_to(window)
        window.render(draw_delay)  # Pauses for 0.05 seconds after rendering.
